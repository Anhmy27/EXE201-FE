    ...
    <div class="container mx-auto max-w-5xl px-4 py-8">
        ...
        <div class="mb-6">
            <Breadcrumb :items="[
                ...
                { name: dataUser?.email , href: '/#', current: true },
...
const fetchOrders = async () => {
    try {
        ...
        const response = await apiService.getAdminOrderUser(route.params.user_id ,params.value)
...
const getProfileDetail = async () => {
    try {
        isLoading.value = true
        const response = await apiService.getUserDetail(route.params.user_id)
        if (response.code === 200) {
